...
async def check_cooldown(user_id, guild_id, command, cooldown_seconds):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
        ...
        if row:
            import time
            now = int(time.time())
            if now - row[0] < cooldown_seconds:
                return cooldown_seconds - (now - row[0])
        return 0

async def set_cooldown(user_id, guild_id, command):
    import time
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""
            INSERT OR REPLACE INTO cooldowns (user_id, guild_id, command, last_used)
            VALUES (?, ?, ?, ?)
        """, (user_id, guild_id, command, int(time.time())))
        await db.commit()
